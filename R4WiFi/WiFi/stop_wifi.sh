...
function stop_water_monitor_console()
  {
    ...
    if [[ $? -eq 0 ]]; then
      ...
      echo -e "Stopped running log" >> "/var/log/WaterLogs/stoped_Arduino R4 WiFi_waterlog-$(date +"%m-%d-%Y_%H:%M").log"
    else
      echo -e "Logs were not running" >> "/var/log/WaterLogs/stoped_Arduino R4 WiFi_waterlog-$(date +"%m-%d-%Y_%H:%M").log"
    fi
}
